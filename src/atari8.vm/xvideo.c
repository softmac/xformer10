...
void PSLPrepare(void *candy)
{
    if (PSL == 0 || PSL >= wSLEnd)    // or just trust the caller and don't check
    {
        ...
        if (wScan == STARTSCAN)
        {
            ...
            PrecomputeArtifacting(candy);   // the best way I know to detect a change from color->B&W is to check every frame
        }
...
void PSLInternal(void *candy, unsigned start, unsigned stop, unsigned i, unsigned iTop, unsigned bbars, RECT *prectTile)
{
    ...
    // We want to artifact if we are a colour TV and if the foreground and background are different luminences (otherwise
    // everything is invisible and it's wrong to artifact, and faster to skip it rather than do calculations that
    // would just result in nothing showing up). Col.colx might be bitfields, so the only way to see the true colours
    // is to look at the COLPFx locations. Also, for mode 2-3, we have to make sure we're not in GTIA mode to artifact

    // TRUE if we want artifacting in normal screen memory mode where we write colour data (!sl.fpmg)
    const BOOL fArtifacting = pvm->bfMon == monColrTV && (COLPF1 & 0xf) != (COLPF2 & 0xf) && !pmg.fGTIA && !sl.fpmg;
    // TRUE if we want artifacting in PMG mode where we are dealing in bit fields and can't actually do the artifacting
    // right now (we'll be storing the data for later)
    const BOOL fPMGA = pvm->bfMon == monColrTV && (COLPF1 & 0xf) != (COLPF2 & 0xf) && !pmg.fGTIA && sl.fpmg; // fill in special array
    ...
    if (sl.modelo < 2 || iTop > i)
    {
        ...
        // GR.0, much of the code is the same as mode 3 but is separated out for performance, since mode 3 is the rarest mode
        // and mode 2 is the most common mode
        case 2:
        {
            vpix = vpix % 8;
            ...
            // apply inverse video and blinking
            if (b1 & 0x80)
                ...
                if (!pmg.fGTIA)
                {
                    ...
                    if (!fArtifacting && !fPMGA)
                    {
                        ...
                        if (sl.fpmg)
                            ...
                            *(ULONG *)qch = (Fill1 & BlendMask) | (Fill2 & ~BlendMask);
                    ...
                    else
                    {
                        ...
                        if (fPMGA)
                        {
                            *(ULONG *)(&rgArtifact[qch - qchStart]) = (((FillA & ~ColorMask) | (Fill1 & ColorMask)) & BlendMask) |
                                (Fill2 & ~BlendMask);
                            *(ULONG *)qch = (((Fill1bf & ~ColorMask) | (Fill1bf & ColorMask)) & BlendMask) | (Fill2bf & ~BlendMask);
                        }
                        else
        ...
        // mostly the same as mode 2, but separated out to avoid if's (4% speed improvment)
        case 3:
        {
            BYTE VPIX1, VPIX2, vpixO = vpix % 10;
            ...
            // precompute vertical reflect values for different characters in the character set
            if (sl.chactl & 4)
            {
                VPIX1 = vpixO < 8 ? 7 - vpixO : vpixO;
                VPIX2 = (vpixO > 7) ? (23 - vpixO) : (vpixO < 2 ? vpixO : (vpixO < 6 ? 7 - vpixO : 15 - vpixO));
            }
            else
            {
                VPIX1 = vpix;
            ...
            // Start by looking at the previous character, we'll always keep info on 2 consecutive characters
            // for hscrol and artifacting
            b1 = cpuPeekB(candy, (wAddr & 0xF000) | ((wAddr + wAddrOff + i - 1) & 0x0FFF));

            // properly vertically reflect this character
            vpix = ((b1 & 0x7F) < 0x60) ? VPIX1 : VPIX2;

            // we are not in the blank part of a mode 3 character, then dereference the character set at the proper row
            WORD vv;
            if ((vpix >= 2 && vpix < 8) || (vpix < 2 && (b1 & 0x7F) < 0x60) ||
                (vpix >= 8 && (b1 & 0x7F) >= 0x60))
            {
                // use the top two rows of pixels for the bottom 2 scan lines for ANTIC mode 3 descended characters
                WORD vpix3 = (vpix >= 8 && (b1 & 0x7f) >= 0x60) ? vpix - 8 : vpix;

                // sl.chbase is already on a proper page boundary
                vv = cpuPeekB(candy, (sl.chbase << 8) + ((b1 & 0x7f) << 3) + vpix3);
            }
            ...
            for (; i < iTop; i++)
            {
                ...
                {
                    // use the top two rows of pixels for the bottom 2 scan lines for ANTIC mode 3 descended characters
                    BYTE vpix3 = (vpix >= 8 && (b1 & 0x7f) >= 0x60) ? vpix - 8 : vpix;

                    // sl.chbase is already on a proper page boundary
                    b2 = cpuPeekB(candy, (sl.chbase << 8) + ((b1 & 0x7f) << 3) + vpix3);
                }
                ...
                // the previous character goes in the high nibble, the current character in the low nibble
                vv = (vv << 8) | b2;

                // now do the shifting, but leave the original intact
                WORD u = vv >> hshift;

                // the common case of not doing undocumented GTIA in mode 3.
                if (!pmg.fGTIA)
                {
                    // We don't want artifacting (B&W, maybe)
                    if (!fArtifacting && !fPMGA)
        ...
        case 6:
            ...
            for (; i < iTop; i++)
            {
                ...
                else
                {
                    ...
                    const ULONG xFill1 = 0x01010101 * Col.col1;

                    ULONG BlendMask = BitsToByteMask[(b2 >> 4) & 0xF];
                    ULONG PixelsHi = (xFill1 & BlendMask) | (Fill0 & ~BlendMask);
                    BlendMask = BitsToByteMask[(b2 >> 0) & 0xF];
                    ULONG PixelsLo = (xFill1 & BlendMask) | (Fill0 & ~BlendMask);
...
#else   // DANNY's algorithm

                    // THEORY OF OPERATION - INTERLACING (Danny's algorithm)
