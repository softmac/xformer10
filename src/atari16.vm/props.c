...
#ifdef XFORMER
char const szIniFile[] = "XF2000.INI";
#else
#if !defined(SOFTMAC) && 1
char const szIniFile[] = "GEM2000.INI";
#else
char const szIniFile[] = "SOFTMAC.INI";
#endif // SOFTMAC
#endif // XFORMER
...
#ifdef TUTOR
    // initialize the default Tutor VM

    FAddVM(&vmiTutor, NULL);
#endif

#endif // !XFORMER

#if defined(WORMHOLE) || defined(DEMO)
...
LTryAgain:
	...
	if ((vTmp.cb == sizeof(vTmp)) && (vTmp.wMagic == v.wMagic))
        ...
        {
        // treat the file name entered as a registration unlock key
        ...
        for(i = sizeof(sz); i > 0; i--)
            {
            if (sz[i-1] == '\\')
                break;
            }

        if (_STS_SUCCESS == _STS_UnlockKey (sz+i, &e))
            {
            memcpy (vpes, &e, sizeof (e));
            _STS_UnlockKey (vpes->key, vpes);

            // set validation bytes so that valid1*valid2 == $FF
...
#ifdef TUTOR
        if (FIsTutor(v.rgvm[i].bfHW))
            v.rgvm[i].pvmi = &vmiTutor;
#endif
    ...
    if (f)
        ...
        // unscramble the encryption data

        for (i = 0; i < sizeof(vpes->key); i++)
            vpes->key[i] ^= ~(1 << (i & 7));
#endif

        // decrypt it

        if (_STS_SUCCESS != _STS_UnlockKey (vpes->key, vpes))
            {
            memset(vpes, 0, sizeof(*vpes));
            }
        }
#endif
...
#if !defined(DEMO) && defined(GEMPRO) && !defined(XFORMER)
    // and preserve the registration key

    memcpy(&v.rgchEncrypt, &vTmp.rgchEncrypt, sizeof(v.rgchEncrypt));
#endif
